...
# Severities that trigger exit code 1 per --fail-on threshold.
# "none" means the scanner always exits 0 (audit-only mode).
_FAIL_SEVERITIES: dict[str, set] = {
    "critical": {"CRITICAL"},
    "high":     {"CRITICAL", "HIGH"},
...
def _phase(n: int, total: int, label: str) -> None:
    print(f"{C_CYAN}[{n}/{total}] {label}...{C_RESET}", file=sys.stderr)


def _phase_result(count: int) -> None:
    colour = C_RED if count else C_GREEN
    noun   = "issue" if count == 1 else "issues"
    print(f"      {colour}Found {count} {noun}{C_RESET}", file=sys.stderr)
...
def _print_summary(findings: list, elapsed: float) -> None:
    ...
    print(f"{C_BOLD}{'=' * 60}{C_RESET}", file=sys.stderr)
    print(f"{C_BOLD}  Scan Summary{C_RESET}", file=sys.stderr)
    ...
    for sev in ("CRITICAL", "HIGH", "MEDIUM", "LOW", "INFORMATIONAL"):
        ...
        if c:
            ...
            print(f"  {color}{sev:<15}{c}{C_RESET}", file=sys.stderr)
    print(f"{C_BOLD}{'=' * 60}{C_RESET}", file=sys.stderr)
    print(file=sys.stderr)
...
def run_scan(args: argparse.Namespace) -> int:
    ...
    fail_sevs = _FAIL_SEVERITIES.get(fail_on, {"CRITICAL", "HIGH"})
    return 1 if any(f.get("severity") in fail_sevs for f in all_findings) else 0
...
Examples:
  ...
  python main.py --url http://target.com --proxy http://127.0.0.1:8080 --format all
    ...
    parser.add_argument("--fail-on",
                        ...
                        dest="fail_on",
                        help=(
                            "Exit code 1 threshold (default: high). "
                            "Use 'none' to always exit 0 — suitable for "
